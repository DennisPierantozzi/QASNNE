...
try:
    from scipy.special import logsumexp as _lse   # best if SciPy is present
except Exception:
    _lse = None

def _safe_logsumexp(x):
    """Stable logsumexp that handles all -inf cleanly."""
    x = np.asarray(x, dtype=np.float64)
    finite = np.isfinite(x)
    if not finite.any():           # all -inf → logsumexp = -inf
        return -np.inf
    m = x[finite].max()
    return m + np.log(np.sum(np.exp(x[finite] - m)))
...
class EntailmentDeberta(BaseEntailment):
    def __init__(self):
        model_path = "/SAN/medic/Cholec/hf/models/microsoft-deberta-v2-xlarge-mnli"
    ...
    def check_implication(self, text1, text2, *args, **kwargs):
        ...
        with torch.no_grad():
            ...
            confidence = torch.max(largest_index)
...
def logsumexp_by_id(semantic_ids, log_likelihoods, agg='sum_normalized', return_unique_ids=False):
    ...
    valid_lls = [ll for ll in log_likelihoods if ll != 0.0]
    if not valid_lls:
...
def predictive_entropy(log_probs):
    ...
    """
    valid_probs = [p for p in log_probs if p != 0.0]
    if not valid_probs:
        return 0.0
